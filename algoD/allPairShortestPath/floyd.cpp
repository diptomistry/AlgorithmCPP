...
void floyd_warshall(int graph[V][V])
{
	...
	//Assign all values of graph to allPairs_SP//copying the graph to allPairs_SP
	for(int i=0;i<V;++i)
		for(int j=0;j<V;++j)
			dist[i][j] = graph[i][j];
	...
	//Check for negative edge weight cycle//we just need to check the self distance values if it is negative
	for(int i=0;i<V;++i)
		if(dist[i][i] < 0)
		{
...
int main()
{
	int graph[V][V] = { {0, 1, 4, INT_MAX, INT_MAX, INT_MAX},//all the 0 s are diagonal values as there is no self loop
