...
void bellmanFord(vector<edge>& Edges)
{
	...
	for(int i=0;i<V-1;++i)
	{
		...
		if(updated==false)//same value as the previous iteration
			break;
	}
	...
	for(int j=0;j<E and updated==true;++j)
		{
			...
			if(value[U]!=INT_MAX and value[U]+wt<value[V])
			{
				cout<<"Graph has -VE edge cycle\n";//if a node is having a new shorter path then there is a cycle
				return;
			}
		}
...
//updated is used to reduce the runtime ,the algo will work without it too but it will take more time
